...
pub async fn generations_response(
    ...
    let processed_body = if !body_bytes.is_empty() {
        ...
        let mut openai_request_json: Value = serde_json::from_slice(&body_bytes)
            .map_err(|e| ProxyError::Json(format!("Failed to parse JSON: {}", e)))?;

        upload_openai_media_inputs(
            &mut openai_request_json,
            &target_base,
            &state.client,
            &headers,
            req_id,
        ...
        let transformed_body = Bytes::from(
            serde_json::to_vec(&openai_request_json)
...
/// Upload image/media inputs from an OpenAI-style request into ComfyUI input storage.
///
/// Supported request fields:
/// - `image`: single image string (URL, base64 data-url, or raw base64)
/// - `images`: array of image strings (for multi-image video workflows)
async fn upload_openai_media_inputs(
    openai_json: &mut Value,
    target_base: &str,
    client: &Client,
    headers: &HeaderMap,
    req_id: &str,
) -> Result<(), ProxyError> {
    let Some(obj) = openai_json.as_object_mut() else {
        return Ok(());
    };

    let mut raw_media_items: Vec<String> = Vec::new();

    if let Some(image) = obj.get("image").and_then(|v| v.as_str()) {
        raw_media_items.push(image.to_string());
    }

    if let Some(images) = obj.get("images").and_then(|v| v.as_array()) {
        for item in images {
            if let Some(v) = item.as_str() {
                raw_media_items.push(v.to_string());
            }
        }
    }

    if raw_media_items.is_empty() {
        return Ok(());
    }

    info!(
        "┃ [{}] 🖼️ Uploading {} input image(s) for workflow",
        req_id,
        raw_media_items.len()
    );
    ...
    for (idx, item) in raw_media_items.iter().enumerate() {
        let media_bytes = resolve_media_bytes(item, client).await?;
        let file_ext = detect_image_extension(&media_bytes);
        let file_name = format!("openai_video_input_{}_{}.{}", req_id, idx, file_ext);

        let upload_url = format!("http://{}/upload/image", target_base);
        let boundary = format!("----comfyui-openai-{}-{}", req_id, idx);
        let body = build_upload_multipart_body(&boundary, &file_name, &media_bytes);
        ...
        let resp = req.send().await.map_err(|e| {
            ProxyError::Upstream(format!("Failed to upload media to ComfyUI: {}", e))
        })?;
        ...
        if !resp.status().is_success() {
            ...
            return Err(ProxyError::Upstream(format!(
                "ComfyUI upload failed with {}: {}",
                st, body
            )));
        }

        let upload_json: Value = resp
            .json()
            .await
            .map_err(|e| ProxyError::Json(format!("Failed to parse upload response: {}", e)))?;

        let uploaded_name = upload_json
            .get("name")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ProxyError::Json("Upload response missing 'name'".to_string()))?;

        uploaded_files.push(uploaded_name.to_string());
    }

    if let Some(first) = uploaded_files.first() {
        obj.insert("image".to_string(), Value::String(first.clone()));
    }

    obj.insert(
        "images".to_string(),
        Value::Array(uploaded_files.iter().cloned().map(Value::String).collect()),
    );

    Ok(())
}

fn build_upload_multipart_body(boundary: &str, file_name: &str, media_bytes: &[u8]) -> Vec<u8> {
    ...
    body.extend_from_slice(part2.as_bytes());
...
async fn resolve_media_bytes(media: &str, client: &Client) -> Result<Vec<u8>, ProxyError> {
    if media.starts_with("http://") || media.starts_with("https://") {
        let bytes = client
            .get(media)
            .send()
            .await
            .map_err(|e| ProxyError::Upstream(format!("Failed to download media URL: {}", e)))?
            .bytes()
            .await
            .map_err(|e| ProxyError::Upstream(format!("Failed to read media URL body: {}", e)))?;
        return Ok(bytes.to_vec());
    }

    let raw_b64 = if media.starts_with("data:") {
        media
            .split_once(",")
            .map(|(_, b64)| b64)
            .ok_or_else(|| ProxyError::Json("Invalid data URL for image input".to_string()))?
    } else {
        media
    };
    ...
    general_purpose::STANDARD
        .decode(raw_b64)
        .map_err(|e| ProxyError::Json(format!("Failed to decode base64 image input: {}", e)))
}
...
fn extract_image_inputs(openai_request: &serde_json::Map<String, Value>) -> Vec<String> {
    let mut out = Vec::new();

    if let Some(v) = openai_request.get("image").and_then(|v| v.as_str()) {
        out.push(v.to_string());
    }

    if let Some(arr) = openai_request.get("images").and_then(|v| v.as_array()) {
        for item in arr {
...
fn inject_image_inputs(inputs: &mut serde_json::Map<String, Value>, image_inputs: &[String]) {
    if image_inputs.is_empty() {
        return;
    }

    if inputs.contains_key("image") {
        inputs.insert("image".to_string(), Value::String(image_inputs[0].clone()));
    }

    if inputs.contains_key("images") {
        inputs.insert(
            "images".to_string(),
            Value::Array(image_inputs.iter().cloned().map(Value::String).collect()),
        );
    }

    for (i, file) in image_inputs.iter().enumerate() {
        let key = format!("image{}", i + 1);
        if inputs.contains_key(&key) {
            inputs.insert(key, Value::String(file.clone()));
        }
    }
}
...
async fn create_json_payload(
    ...
    if let Some(openai_request) = json.as_object() {
        let image_inputs = extract_image_inputs(openai_request);
                ...
                for (_node_id, node_data) in workflow_prompt {
                    if let Some(class_type_ref) = node_data["class_type"].as_str() {
                        ...
                        match class_type.as_str() {
                            ...
                            // Handle image inputs for img2video / multi-image video workflows
                            "LoadImage" | "LoadImageMask" | "VHS_LoadImagePath"
                            | "VHS_LoadImagesPath" | "LoadImageFromPath" => {
                                if let Some(inputs_data_image) = node_data["inputs"].as_object_mut()
                                {
                                    inject_image_inputs(inputs_data_image, &image_inputs);
                                }
                            }
                            // Handle Text Prompts
